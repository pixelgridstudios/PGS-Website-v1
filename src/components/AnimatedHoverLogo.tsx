  ...
  return (
    ...
    >
      ...
      <img
        ...
        alt="Pixel Grid"
        className={`h-[26px] w-auto sm:h-[28px] dark:invert object-contain select-none shrink-0 transition-opacity duration-300 ${isHovered ? "opacity-0" : "opacity-100"}`}
      />
      {/* Animated Sequence Frames on Hover */}
      <img
        src={`/assets/pg-emblem-frames/frame_${currentFrame}.png`}
        alt="Pixel Grid Animated"
        className={`absolute left-0 bottom-0 h-[22px] w-auto sm:h-[27px] max-w-none invert dark:invert-0 object-[left_bottom] object-contain select-none shrink-0 origin-bottom-left transition-opacity duration-300 pointer-events-none ${isHovered || currentFrame > 0 ? "opacity-100" : "opacity-0"}`}
      />
    </Link>
  );
};
